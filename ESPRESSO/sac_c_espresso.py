...
initial_key_stream_init = [0]* itter
n = 1024/32
for i in range(0,itter):
    initial_key_stream_init[i] = espresso_function(iv_init[i],key_init[i],n)
...
for ijk in range (changes):
    for ppp in range(itter):
        ...
        initial_key_stream = initial_key_stream_init[ppp] 
        ...
        #print 'IV used : ', iv_temp_c
        if(key_temp_c[-1] == 'L'):
            iv_temp_c = iv_temp_c[:-1]
        if(key_temp_c[-1] == 'L'):
            key_temp_c = key_temp_c[:-1]


        output[ppp] = espresso_function(iv_temp_c,key_temp_c,n) ^ initial_key_stream
        #sprint output[ppp]
        #print 'out ',ppp,' : ', output[ppp]
    ...
    for i in range (1024):
        for j in range (itter):
            relation_factor[i] = relation_factor[i]+ int(output[j][i]);
