...
for ppp in range(0,itter):
    ...
    for i in range(0,K):
        iv[i] = os.urandom(8).encode('hex')
        iv_s = str_to_blocks(iv[i],8)
        key_s = str_to_blocks("0",10)
        ...
        if(relation_factor[i] <= 30 or relation_factor[i] >= 90):
            print "Error --> Weight = " + str(relation_factor[i]) + " " + "IV Used: " + str(iv[i])
        if(i % (1024*32) == 0):
